            ...
            do
                ...
                do
                    ...
                    do
                        ...
                        do 
                            echo "[$(date +%T)][INFO] RUNNING: python3 test_topo.py ${RESULT_DIR}/exp_loadLimit_${topo_load_limit}_behavioral_${topo_behave} ${TOPO_DIR}/${topo_name}-${topo_num_node}-${topo_dregree}/seed_${topo_seed}/ ${topo_seed} ${topo_criterion} ${topo_behave} ${topo_load_limit} ${TOPO_RUNS}"
                            python3 test_topo.py ${RESULT_DIR}/exp_loadLimit_${topo_load_limit}_behavioral_${topo_behave} ${TOPO_DIR}/${topo_name}-${topo_num_node}-${topo_dregree}/seed_${topo_seed}/ ${topo_seed} ${topo_criterion} ${topo_behave} ${topo_load_limit} ${TOPO_RUNS}
                        done
                    done
                done
            done
