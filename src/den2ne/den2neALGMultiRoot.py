...
class Den2neMultiRoot(object):
                    ...
                    if not curr_node.ids[i].used:
                        ...
                        for neighbor in curr_node.neighbors:
                            ...
                            # elif len(self.G.nodes[neighbor].ids) >= Den2neMultiRoot.IDS_MAX:
                                ...
                                self.G.nodes[neighbor].ids_root_count
                                >= self.MAX_IDS_X_ROOT
                            ):
                                ...
                                if id_switch_node == id_switch_neighbor:
                                    self.G.nodes[neighbor].ids.append(
                                        HLMAC(
                                            curr_node.ids[i], neighbor, id_switch_node
                                        )
                                    )
                                else:
                                    self.G.nodes[neighbor].ids.append(
    ...
    def flowInertia(self, roots_closed=None, ids_to_fix=None, n_repetition=None):
        ...
        if (
            ...
            self.global_ids.sort(
                key=Den2neMultiRoot.key_sort_by_HLMAC_len, reverse=True
            )
            ids_list = [
                j
                                    ...
                                    if len(curr_node.getActiveID().hlmac) <= len(
                                        ...
                                        for id in curr_node.ids:
                                            if all(
                                                ...
                                                if (
                                                    roots_closed == None
                                                    or id.hlmac[0] not in roots_closed
                                                ):
                                                    possible_id.append(id)
                                        if possible_id:
                                            possible_id.sort(
                                                key=Den2neMultiRoot.key_sort_by_HLMAC_len
    ...
    def selectBestIDs(self, criterion, roots_closed=None):
        ...
        elif Den2neMultiRoot.CRITERION_POWER_TO_ZERO == criterion:
            self.selectBestID_by_power2zero(roots_closed)

        elif Den2neMultiRoot.CRITERION_POWER_TO_ZERO_WITH_LOSSES == criterion:
            self.selectBestID_by_power2zero_with_Losses(roots_closed)
    ...
    def selectBestID_by_hops(self, roots_closed):
        ...
        for node in self.G.nodes:
            if roots_closed != None:
                lens_without_closed_roots = [
                    len(id.hlmac)
                    for id in self.G.nodes[node].ids
                    if id.hlmac[0] not in roots_closed
                ]
                lens = [len(id.hlmac) for id in self.G.nodes[node].ids]
                for correct_index in [
                    posible_index
                    for posible_index in range(len(lens))
                    if lens[posible_index] == min(lens_without_closed_roots)
                ]:
                    if (
                        self.G.nodes[node].ids[correct_index].hlmac[0]
                        not in roots_closed
                    ):
    ...
    def selectBestID_by_distance(self, roots_closed):
        ...
        for node in self.G.nodes:
            ...
            if roots_closed != None:
                dists_without_closed_roots = [
                    self.getTotalDistance(id)
                    for id in self.G.nodes[node].ids
                    if id.hlmac[0] not in roots_closed
                ]
                ...
                for correct_index in [
                    posible_index
                    for posible_index in range(len(dists))
                    if dists[posible_index] == min(dists_without_closed_roots)
                ]:
                    if (
                        self.G.nodes[node].ids[correct_index].hlmac[0]
                        not in roots_closed
    ...
    def selectBestID_by_balance(self, roots_closed):
        ...
        for node in self.G.nodes:
            if roots_closed != None:
                balances_without_closed_roots = [
                    self.getTotalBalance(id)
                    for id in self.G.nodes[node].ids
                    if id.hlmac[0] not in roots_closed
                ]
                ...
                for correct_index in [
                    posible_index
                    for posible_index in range(len(balances))
                    if balances[posible_index] == max(balances_without_closed_roots)
                ]:
                    if (
                        self.G.nodes[node].ids[correct_index].hlmac[0]
                        not in roots_closed
    ...
    def selectBestID_by_balance_with_Losses(self, roots_closed):
        ...
        for node in self.G.nodes:
            if roots_closed != None:
                balances_without_closed_roots = [
                    self.getTotalBalance_with_Losses(id)
                    for id in self.G.nodes[node].ids
                    if id.hlmac[0] not in roots_closed
                ]
                balances = [
    ...
    def selectBestID_by_Links_Losses(self, roots_closed):
        ...
        for node in self.G.nodes:
            if roots_closed != None:
                losses_without_closed_roots = [
                    self.getTotalLinks_Losses(id)
                    for id in self.G.nodes[node].ids
                    if id.hlmac[0] not in roots_closed
                ]
                losses = [
                    self.getTotalLinks_Losses(id) for id in self.G.nodes[node].ids
                ]
                for correct_index in [
                    posible_index
                    for posible_index in range(len(losses))
                    if losses[posible_index] == min(losses_without_closed_roots)
                ]:
                    if (
                        self.G.nodes[node].ids[correct_index].hlmac[0]
                        not in roots_closed
    ...
    def selectBestID_by_weighted_balance(self, roots_closed):
        ...
        for node in self.G.nodes:
            if roots_closed != None:
                norm_balances_without_closed_roots = [
                    self.getTotalWeightedBalance(id)
                    for id in self.G.nodes[node].ids
                    if id.hlmac[0] not in roots_closed
                ]
                norm_balances = [
                ...
                for correct_index in [
                    posible_index
                    for posible_index in range(len(norm_balances))
                    if norm_balances[posible_index]
                    == max(norm_balances_without_closed_roots)
                ]:
                    # print(correct_index)
                    if (
                        self.G.nodes[node].ids[correct_index].hlmac[0]
    ...
    def selectBestID_by_power2zero(self, roots_closed):
        """
        Función para decidir la mejor ID de un nodo cercanía de potecia a cero, al root
        """
        for node in self.G.nodes:
            if roots_closed != None:
                # TODO logic with multiples iterations
                pass
            else:
                power2zero = [
                    self.getTotalPower2Zero(id) for id in self.G.nodes[node].ids
                ]
    ...
    def selectBestID_by_power2zero_with_Losses(self, roots_closed):
        """
